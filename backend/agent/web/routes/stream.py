...
def _use_langchain_v1_runtime(
    *,
    agent_runtime: Optional[str] = None,
    use_langchain_v1: Optional[bool] = None,
) -> bool:
    """Resolve the optional LangChain 1.x runtime switch."""
    if use_langchain_v1 is not None:
        return bool(use_langchain_v1)

    runtime = (agent_runtime or os.getenv("AGENT_RUNTIME", "")).strip().lower()
    return runtime in {"langchain_v1", "langchain1", "lc1"}
...
async def _stream_event_generator(
    ...
    try:
        ...
        if _use_langchain_v1_runtime(
            agent_runtime=agent_runtime,
            use_langchain_v1=use_langchain_v1,
        ):
            from backend.agent.langchain_v1 import LangChainV1ResumeRunner

            runner = LangChainV1ResumeRunner(
                session_id=conversation_id,
                agent=agent,
...
async def stream_events(
    ...
    return StreamingResponse(
        _stream_event_generator(
            ...
            use_langchain_v1=request.use_langchain_v1,
        ),
