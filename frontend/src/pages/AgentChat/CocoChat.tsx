...
function resolveResumeDataId(data: ResumeData | null | undefined): string | null {
  const candidate =
    data?.resume_id ??
    data?._meta?.resume_id ??
    data?.id;
...
function resumeDataMatchesActiveId(
  data: ResumeData | null | undefined,
  activeId: string | null | undefined,
): boolean {
  const dataId = resolveResumeDataId(data);
  const normalizedActiveId =
    typeof activeId === "string" && activeId.trim() ? activeId.trim() : null;
  return !dataId || !normalizedActiveId || dataId === normalizedActiveId;
}
...
function CocoChatContent() {
  ...
  useEffect(() => {
    if (!contextResume) return;

    const activeId =
      selectedResumeId || resumeId || resolveResumeDataId(resumeDataRef.current);
    if (!resumeDataMatchesActiveId(contextResume, activeId)) return;

    resumeDataRef.current = contextResume;
    setResumeData((prev) => (prev === contextResume ? prev : contextResume));

    const contextResumeId = resolveResumeDataId(contextResume);
    const targetId = contextResumeId || selectedResumeId || resumeId;
    if (targetId) {
      setLoadedResumes((prev) => {
        ...
        const next = prev.map((item) => {
          ...
          return {
            ...item,
            name: contextResume.basic?.name || contextResume.title || item.name,
            resumeData: contextResume,
          };
        });
    ...
    setCarryResumePrompt((prev) => {
      if (!prev) return prev;
      const promptId = prev.id;
      if (targetId && promptId !== targetId) return prev;
      return {
        ...prev,
        name: contextResume.basic?.name || contextResume.title || prev.name,
        data: contextResume,
      };
    });
  }, [contextResume, resumeId, selectedResumeId]);
  ...
  const sendUserTextMessage = useCallback(
    async (
      ...
      const activeResumeId =
        selectedResumeId || resumeId || resolveResumeDataId(resumeDataRef.current);
      const latestContextResume =
        contextResume && resumeDataMatchesActiveId(contextResume, activeResumeId)
          ? contextResume
          : null;
      const resumeDataForThisMessage =
        resumeDataOverride !== undefined
          ? resumeDataOverride
          : latestContextResume ?? resumeDataRef.current;
      if (resumeDataForThisMessage && resumeDataRef.current !== resumeDataForThisMessage) {
        resumeDataRef.current = resumeDataForThisMessage;
      }
      ...
      try {
        ...
        for (const file of attachmentsToProcess) {
          ...
          if (isPdf) {
            ...
            if (!response.ok) {
              throw new Error(`PDF 解析失败: ${await readUploadError(response)}`);
            }
